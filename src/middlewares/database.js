...
export class Database {
    ...
    //persistir os dados quando a aplicação inicializa:
    constructor() { // metodo que vai ser executado assim que o BD for instanciado
        fs.readFile(databasePath, 'utf8') // utf8 é o encoding que está sendo utilizado
        .then(data => {
            this.#database = JSON.parse(data) // salvando os dados dentro do BD
        })
        .catch(()=> { // caso esse arquivo não existir (se não tiver feito a requisição ainda, por exemplo)
            this.#persist() // vai chamar o método persist só para criar esse arquivo (mesmo que vazio)
        })
    }
